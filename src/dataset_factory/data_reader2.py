...
def _tsn_data_augment(sampled_images, label):
  ...
  if num_segments*new_length != len(sampled_images):
    raise ValueError("batch size error!")
  img_h=video_batch.get_shape().as_list()[1]
  img_w=video_batch.get_shape().as_list()[2]
  boxes=[]
  box_ind=[]
  num=0
  # 对sampled_images，整体做一个crop
  for i in range(cfg.NUM_SEGMENTS*cfg.NEW_LENGTH):
...
class TSNDataReader():
  ...
  def get_batch(self):
    ...
    # 预处理map
    dataset=dataset.map(_tsn_data_argument,num_parallel_calls=4) 
    # shuffle, get_batch
